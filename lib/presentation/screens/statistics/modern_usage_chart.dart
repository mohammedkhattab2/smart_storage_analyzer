    ...
    List<int> getSmartXLabels() {
      ...
      if (period.contains('Week')) {
        ...
        for (int i = 0; i < total; i++) {
          final wd = dataPoints[i].date.weekday; 
          if (!seenWeekdays.contains(wd)) {
            labels.add(i);
            seenWeekdays.add(wd);
          }
        }
        ...
        int step = (total / 4).ceil();
        for (int i = 0; i < total; i += step) {
          labels.add(i);
        }
        if (!labels.contains(total - 1)) labels.add(total - 1);
      } else if (period.contains('Year')) {
        
        final seenMonths = <int>{};
        for (int i = 0; i < total; i++) {
          final m = dataPoints[i].date.month;
        ...
        int count = total <= 7 ? total : 7;
        double step = (total - 1) / (count - 1);
        for (int i = 0; i < count; i++) {
          labels.add((i * step).round());
        }
      }
                          ...
                          sideTitles: SideTitles(
                            ...
                            getTitlesWidget: (value, meta) {
                              ...
                              if (period.contains('Week')) {
                                ...
                                final weekNum = ((index / step).ceil()).clamp(1, 4);
                                label = 'Week $weekNum';
                              } else if (period.contains('Year')) {
